        ...
        for filt in filtlist:
            ...
            if len(shifted_mjd) > 0:
                ...
                for i, phase in enumerate(phases):
                    ...
                    if plot:
                        plt.errorbar(phase, mags[i] - mag_grid[phase_ind, wl_ind], yerr=np.sqrt(errs[i]**2 + err_grid[phase_ind, wl_ind]**2), marker='o', color='k')
                        plt.errorbar(phase, mags[i], yerr=errs[i], fmt='o', color=colors.get(filt, 'k'))
